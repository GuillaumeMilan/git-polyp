...
fn run_convert(verbose: bool) {
    let current_dir = match std::env::current_dir() {
        Ok(dir) => dir,
        Err(e) => {
            eprintln!("{}", format!("Failed to get current directory: {}", e).deco_as_error());
            std::process::exit(1);
        }
    };
    ...
    if detect::is_worktree_root(&current_dir) {
        eprintln!("{}", "Already in a git-polyp worktree workspace.".deco_as_error());
        std::process::exit(1);
    }
    ...
    if !git_dir.exists() {
        eprintln!("{}", "Not in a git repository root. The .git directory must exist in the current directory.".deco_as_error());
        std::process::exit(1);
    }
    ...
    let entries_to_move: Vec<_> = match std::fs::read_dir(&current_dir) {
        Ok(entries) => entries
            ...
            .filter(|e| {
                ...
                name_str != ".git" && name_str != ".bare" && name_str != ".template" && name_str != metadata::METADATA_FILENAME
            })
            .collect(),
        Err(e) => {
            eprintln!("{}", format!("Failed to read directory: {}", e).deco_as_error());
            std::process::exit(1);
        }
    };
    ...
    if let Err(e) = std::fs::rename(&git_dir, &bare_path) {
        eprintln!("{}", format!("Failed to move .git to .bare: {}", e).deco_as_error());
        std::process::exit(1);
    }
    ...
    if let Ok(config_content) = std::fs::read_to_string(&config_path) {
        ...
        if let Err(e) = std::fs::write(&config_path, updated_config) {
            eprintln!("{}", format!("Warning: Failed to update git config: {}", e).bright_yellow());
        }
    }

    // Step 3: Create worktree directory for main branch
    let worktree_dir = current_dir.join(&main_branch);
    println!("Creating worktree directory for '{}'...", main_branch.bright_cyan());

    if let Err(e) = std::fs::create_dir(&worktree_dir) {
        // Restore .git on failure
        let _ = std::fs::rename(&bare_path, &git_dir);
        eprintln!("{}", format!("Failed to create worktree directory: {}", e).deco_as_error());
        std::process::exit(1);
    }
    ...
    for entry in entries_to_move {
        ...
        if let Err(e) = std::fs::rename(&source, &dest) {
            eprintln!("{}", format!("Warning: Failed to move {}: {}", source.display(), e).bright_yellow());
        }
    }
    ...
    if let Err(e) = std::fs::create_dir(&template_path) {
        eprintln!("{}", format!("Warning: Failed to create .template directory: {}", e).bright_yellow());
    }
    ...
    if let Err(e) = metadata_obj.save(&current_dir) {
        eprintln!("{}", format!("Warning: Failed to create metadata file: {:?}", e).bright_yellow());
    }

    // Step 7: Setup the worktree properly with git
    // Change to .bare directory to run git worktree commands
    if let Err(e) = std::env::set_current_dir(&bare_path) {
        eprintln!("{}", format!("Failed to change to .bare directory: {}", e).deco_as_error());
        std::process::exit(1);
    }
    ...
    if let Err(e) = std::fs::write(&worktree_git_file, &git_file_content) {
        eprintln!("{}", format!("Warning: Failed to create .git file in worktree: {}", e).bright_yellow());
    }
    ...
    if let Err(e) = std::fs::create_dir_all(&worktrees_dir) {
        eprintln!("{}", format!("Warning: Failed to create worktrees directory: {}", e).bright_yellow());
    }

    let worktree_info_dir = worktrees_dir.join(&main_branch);
    if let Err(e) = std::fs::create_dir_all(&worktree_info_dir) {
        eprintln!("{}", format!("Warning: Failed to create worktree info directory: {}", e).bright_yellow());
    }
    ...
    if let Err(e) = std::fs::write(&gitdir_file, format!("{}\n", worktree_git_file.display())) {
        eprintln!("{}", format!("Warning: Failed to create gitdir file: {}", e).bright_yellow());
    }
    ...
    if let Err(e) = std::fs::write(&head_file, format!("ref: refs/heads/{}\n", main_branch)) {
        eprintln!("{}", format!("Warning: Failed to create HEAD file: {}", e).bright_yellow());
    }
    ...
    if let Err(e) = std::fs::write(&worktree_git_file, &proper_git_file_content) {
        eprintln!("{}", format!("Warning: Failed to update .git file in worktree: {}", e).bright_yellow());
    }
    ...
    println!("\nStructure:");
    println!("  {}/", current_dir.file_name().unwrap_or_default().to_string_lossy().bright_blue());
    println!("    {}     (main branch worktree)", main_branch.bright_cyan());
    println!("    {}            (bare repository)", ".bare".bright_yellow());
    println!("    {}         (template files)", ".template".bright_yellow());
    println!("    {} (metadata)", metadata::METADATA_FILENAME.bright_yellow());
...
fn run_add(branch: &str, base: Option<&str>, verbose: bool) {
    ...
    if remote_exists {
        ...
        if let Err(e) = client::fetch_branch(branch, &verbose) {
            let _ = std::env::set_current_dir(&original_dir);
            eprintln!("{}", format!("Failed to fetch branch from remote: {:?}", e).deco_as_error());
            std::process::exit(1);
        }
    }
...
fn run_switch(branch: &str, verbose: bool) {
    ...
    if !metadata.has_worktree(branch) {
        ...
        if !local_exists && !remote_exists {
            let _ = std::env::set_current_dir(&original_dir);
            eprintln!("{}", format!("Branch '{}' not found locally or on remote.", branch).deco_as_error());
            std::process::exit(1);
        }
        ...
        eprintln!("{}", format!("Created worktree for branch '{}'", branch).bright_green());
    }
...
fn run_prune(verbose: bool) {
    ...
    // Show candidates
    println!("Found {} worktree(s) that can be pruned:\n", candidates.len());
    for (i, candidate) in candidates.iter().enumerate() {
        println!("  {}. {} ({})", i + 1, candidate.name.bright_cyan(), candidate.reason.bright_yellow());
    }
    println!();
    ...
    for candidate in &candidates {
        ...
        if worktree_path.exists() {
            if let Err(e) = client::worktree_remove(worktree_path_str, true, &verbose) {
                eprintln!("{}", format!("  Failed to remove worktree: {:?}", e).bright_yellow());
                continue;
            }
        }
        ...
        match YNQuestion::new(format!("Also delete branch '{}'?", candidate.name)).ask() {
            Ok(true) => {
                if let Err(e) = client::delete_branch(&candidate.name, true, &verbose) {
                    eprintln!("{}", format!("  Warning: Failed to delete branch: {:?}", e).bright_yellow());
                } else {
...
fn run_check(verbose: bool) {
    ...
    for name in worktree_names {
        ...
        // Check if worktree directory exists
        if !worktree_path.exists() {
            println!("  {} {} worktree directory missing", name_display.bright_cyan(), "[ERROR]".bright_red());
            has_issues = true;
            continue;
        }
        ...
        match client::worktree_is_valid(worktree_path_str, &verbose) {
            Ok(false) => {
                println!("  {} {} worktree is broken", name_display.bright_cyan(), "[ERROR]".bright_red());
                has_issues = true;
                continue;
            }
            Err(_) => {
                println!("  {} {} could not verify worktree", name_display.bright_cyan(), "[ERROR]".bright_red());
                has_issues = true;
                continue;
            }
            Ok(true) => {}
        ...
        match client::worktree_has_conflicts(worktree_path_str, &verbose) {
            Ok(true) => {
                println!("  {} {} merge/rebase/cherry-pick in progress", name_display.bright_cyan(), "[WARN]".bright_yellow());
                has_issues = true;
                continue;
            }
            Ok(false) => {}
            Err(_) => {
                println!("  {} {} could not check for conflicts", name_display.bright_cyan(), "[WARN]".bright_yellow());
                has_issues = true;
                continue;
            }
        }
        ...
        match client::worktree_current_branch(worktree_path_str, &verbose) {
            Ok(None) => {
                println!("  {} {} detached HEAD (expected branch {})", name_display.bright_cyan(), "[WARN]".bright_yellow(), entry.branch);
                has_issues = true;
            }
            Ok(Some(current)) => {
                if current == entry.branch {
                    println!("  {} {} on branch {}", name_display.bright_cyan(), "[OK]".bright_green(), entry.branch);
                } else {
                    println!("  {} {} expected branch {}, currently on {}", name_display.bright_cyan(), "[WARN]".bright_yellow(), entry.branch, current);
                    has_issues = true;
                }
            }
            Err(_) => {
                println!("  {} {} could not determine current branch", name_display.bright_cyan(), "[WARN]".bright_yellow());
                has_issues = true;
            }
        }
    }
